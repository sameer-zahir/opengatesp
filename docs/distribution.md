...
Compress-Archive -Path OpenGateSP.exe,Start-OpenGateSP.cmd,README.md,LICENSE,CHANGELOG.md,module,gui,tools,docs,scripts -DestinationPath dist\OpenGateSP-0.11.0.zip -Force
# (run from the repo root after copying dist\OpenGateSP.exe to the repo root, or adjust paths)

gh release create v0.11.0 dist\OpenGateSP-Setup.exe dist\OpenGateSP-0.11.0.zip `
  --title "OpenGateSP 0.11.0" --notes-file release-notes.md
```
